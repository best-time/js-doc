...
Zbase.prototype.windowResize = function(fun) {
  for (var t = 0; t < this.elements.length; t++) {
    ...
    _addEvent(window, "resize", function() {
      ...
      if (
        ele.offsetLeft >
        getInner().width + getScroll().left - ele.offsetWidth
      ) {
        ele.style.left =
          getInner().width + getScroll().left - ele.offsetWidth + "px";
      ...
      if (
        ele.offsetTop >
        getInner().height + getScroll().top - ele.offsetHeight
      ) {
        ele.style.top =
          getInner().height + getScroll().top - ele.offsetHeight + "px";
...
Zbase.prototype.animate = function(obj) {
  for (var i = 0; i < this.elements.length; i++) {
    var ele = this.elements[i];
    var attr =
      obj["attr"] == "x" ? "left" : obj["attr"] == "y" ? "top" : "left"; // 给一个默认值，默认 动画左右移动
...
Zbase.prototype.getInnerText = function(element) {
  return typeof element.textContent == "strign"
    ? element.textContent
    : element.innerText;
};
/**
