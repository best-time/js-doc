(function (name, context, definition) {
  ...
  var win = window,
    ...
    noscope = ['_', '', 0, 1],
    tagMap = {
      // tags that we have trouble *inserting*
      thead: table,
      tbody: table,
      tfoot: table,
      colgroup: table,
   ...
   */
  function data(el) {
    el[getAttribute]('data-node-uid') ||
      el[setAttribute]('data-node-uid', ++uuids);
  ...
  function insert(target, host, fn, rev) {
    var i = 0,
      ...
      nodes =
        query && typeof target == 'string' && target.charAt(0) != '<'
          ? query(target)
          : target;
  ...
  function scroll(x, y, type) {
    ...
    if (x == null && y == null) {
      return (isBody(el)
        ? getWindowScroll()
        : {x: el.scrollLeft, y: el.scrollTop})[type];
    }
    if (isBody(el)) {
      win.scrollTo(x, y);
    } else {
  ...
  Bonzo.prototype = {
    ...
    html: function (h, opt_text) {
      var method = opt_text ? 'textContent' : 'innerHTML',
        that = this,
        ...
        updateElement = function (el, i) {
          try {
            if (
              opt_text ||
              (typeof h == 'string' && !specialTags.test(el.tagName))
            ) {
              return (el[method] = h);
            }
          } catch (e) {}
    ...
    clone: function (opt_host) {
      ...
      for (i = 0, l = this.length; i < l; i++)
        ret[i] = cloneNode(opt_host || this, this[i]);
      return bonzo(ret);
    },
    ...
    toggle: function (opt_callback, opt_type) {
      ...
      return this.each(function (el) {
        el.style.display =
          el.offsetWidth || el.offsetHeight ? 'none' : opt_type;
    ...
    offset: function (opt_x, opt_y) {
      if (
        opt_x &&
        typeof opt_x == 'object' &&
        (typeof opt_x.top == 'number' || typeof opt_x.left == 'number')
      ) {
        return this.each(function (el) {
    ...
    dim: function () {
      ...
      var el = this[0],
        ...
        orig =
          !de && !!el.style && !el.offsetWidth && !el.offsetHeight
    ...
    attr: function (k, opt_v) {
      ...
      return typeof opt_v == 'undefined'
        ...
        : this.each(function (el) {
            specialAttributes.test(k)
              ? (el[k] = setter(el, opt_v))
              : el[setAttribute](k, setter(el, opt_v));
          });
    },
      ...
      if (typeof opt_v === 'undefined') {
        ...
        if (typeof opt_k === 'undefined') {
          each(el.attributes, function (a) {
            (m = ('' + a.name).match(dattr)) &&
              (o[camelize(m[1])] = dataValue(a.value));
          });
          return o;
        } else {
  ...
  bonzo.firstChild = function (el) {
    for (
      var c = el.childNodes, i = 0, j = (c && c.length) || 0, e;
      i < j;
      i++
    ) {
